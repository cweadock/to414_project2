...
log_pred_ME_binary <- ifelse(log_pred_ME >= 0.15, 1, 0) # have to find the right threshold here; may be useful to find break even rates


library(caret)
confusionMatrix(as.factor(log_pred_ME_binary) , as.factor(log_test$yyes), positive = "1")
```
...
knn_model <- knn(tele_train , tele_test , cl = tele_train_labels, k = 1)
...
#This is the code for how to retrieve model from RDS file for prediction


#knn_test <- readRDS("testfile.rds")

#library(gmodels)


#CrossTable(knn_test , tele_test_labels)
